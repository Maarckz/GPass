...
def recuperar_senhas(arquivo_ofuscado, passphrase):
    ...
    for id, senha_ofuscada in senhas_ofuscadas.items():
        ...
        data_list.append({ 'IP': dados_original['IP'], 'Login': dados_original['L'], 'Senha': dados_original['P']})
    ...
    # for id, senha_ofuscada in senhas_ofuscadas.items():
    #     dados_original = decifrar_dados_ofuscados(senha_ofuscada, passphrase)
    #     print(f"ID: {id}, IP: {dados_original['IP']}, Login: {dados_original['L']}, Senha original: {dados_original['P']}")
